...
import render25d, { type Stage2h1Animation } from '@app/stages/Stage2h1/render25d';
import defaultSettings from '@app/stages/Stage2h1/settings';

interface StageProps {
  part?: number;
}

type Stage2h1Settings = Settings & {
  animation: Stage2h1Animation;
};
...
const Stage: Component<StageProps> = (props) => {
  ...
  const [settings, setSettings] = createSignal<Stage2h1Settings>({
    ...defaultSettings,
    animation: {
      delay: 1_000,
  ...
  const updateAnimation = (mode: Stage2h1Animation['mode']) => {
    resolveNextStep();
    runId += 1;
