...
import render25d, { type Stage2h2Animation } from '@app/stages/Stage2h2/render25d';
import defaultSettings from '@app/stages/Stage2h2/settings';
...
const Stage: Component<StageProps> = (props) => {
  ...
  const [settings, setSettings] = createSignal<Stage2h2Settings>({
    ...defaultSettings,
    animation: {
      delay: 1_000,
  ...
  const updateAnimation = (mode: Stage2h2Animation['mode']) => {
    resolveNextStep();
    runId += 1;
