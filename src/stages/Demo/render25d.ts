...
function drawTexturedVerticalLine(
  ...
  const from = minmax(Math.floor(topY), 0, screenHeight);
  const to = minmax(Math.ceil(bottomY), 0, screenHeight);
  ...
  for (let x = xFrom; x <= xTo; x++) {
    ...
    if (wallTexture) {
      ...
      drawTexturedVerticalLine(
        ctx, x, camera.screen.height, drawTop, drawBottom, 
    ...
    if (portalWallType === 'upper' || portalWallType === 'both') {
      ...
      if (wallTop < wallBottom && upperWallHeight > 0) {
        if (wallTexture) {
          ...
          drawTexturedVerticalLine(
            ctx, x, camera.screen.height, wallTop, wallBottom, 
            wallTexture, textureOffsetX, textureStartY, 1 / wallTexture.scale
          );
        } else {
