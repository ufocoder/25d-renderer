...
async function buildBSPTreeRecursive(
  segs: Seg[],
  allSegs: Map<number, Seg>,
...
): Promise<{ success: boolean; node?: BSPNode; newSegments?: Seg[] }> {
  ...
  if (onSplitDebug) {
    await onSplitDebug({ splitter, frontSegs, backSegs });
  }
  ...
  const leftResult = await buildBSPTreeRecursive(
    frontSegs,
    allSegs,
    newUsedSplitterIds,
    frontSector,
  ...
  const rightResult = await buildBSPTreeRecursive(
    backSegs,
    allSegs,
    newUsedSplitterIds,
    backSector,
...
export async function buildBSPTree(
  ...
  onSplitDebug?: (data: any) => Promise<void>
): Promise<BSPNode> {
  resetSegIdCounter();
  ...
  const result = await buildBSPTreeRecursive(
    segsWithId,
    allSegs,
    usedSplitterIds,
    null,
